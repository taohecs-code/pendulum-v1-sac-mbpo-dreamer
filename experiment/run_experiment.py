"""
Unified experiment runner for continuous-control environments.

Supported algorithms:
- SAC (model-free baseline)
- MBPO (model-based policy optimization with synthetic rollouts)
- Dreamer (simplified Dreamer-v1 style latent imagination)

Design goals:
- Same evaluation protocol across algorithms
- Same logging schema (W&B-friendly)
- Same checkpoint naming rules (step / converged / best / final)
- Environment-agnostic CLI via `--env` (e.g., Pendulum, LunarLanderContinuous, MountainCarContinuous)

Important update:
- Previously, checkpoint artifacts were named only by `run.name`, which could collide across
  repeated run names (especially across different environments). This has now been fixed by
  including `wandb.run.id` in artifact names to guarantee uniqueness.
"""
...
# Allow running this script directly via `python experiment/run_experiment.py`
...
def log_checkpoint_artifact(
    ...
    *,
    ...
    # One artifact stream per run-id; avoids collisions for repeated run names across environments.
    art_name = f"{wandb.run.name}-{wandb.run.id}-ckpt"
